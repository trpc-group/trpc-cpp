...
DEFINE_string(service_name, "trpc.test.slidingwindow.Greeter", "callee service name");

int DoRpcCall(const std::shared_ptr<::trpc::test::slidingwindow::GreeterServiceProxy>& proxy, int index) {
  ::trpc::ClientContextPtr client_ctx = ::trpc::MakeClientContext(proxy);
  ::trpc::test::slidingwindow::HelloRequest req;
  req.set_msg("fiber, index=" + std::to_string(index));
  ::trpc::test::slidingwindow::HelloReply rsp;
...
int Run() {
  auto proxy = ::trpc::GetTrpcClient()->GetProxy<::trpc::test::slidingwindow::GreeterServiceProxy>(FLAGS_service_name);

  int index = 0;
  DoRpcCall(proxy, index++);
