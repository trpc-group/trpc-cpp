...
class HelloWorldStreamServer : public ::trpc::TrpcApp {
 public:
  int Initialize() override {
    ...
    std::string service_name = fmt::format("{}.{}.{}.{}", "trpc", config.app, config.server, "AsyncStreamGreeter");
    TRPC_FMT_INFO("service name:{}", service_name);
    RegisterService(service_name, std::make_shared<::test::helloworld::AsyncStreamGreeterServiceImpl>());
