...
DEFINE_string(service_name, "trpc.test.helloworld.StreamForward", "callee service name");
DEFINE_string(client_config, "trpc_cpp_fiber.yaml", "");
DEFINE_string(addr, "127.0.0.1:24759", "ip:port");
DEFINE_string(rpc_method, "ClientStreamSayHello", "RPC method name");
DEFINE_int32(request_count, 3, "count of request");

namespace test::helloworld {

using StreamForwardServiceProxy = ::trpc::test::helloworld::StreamForwardServiceProxy;
using StreamForwardServiceProxyPtr = std::shared_ptr<StreamForwardServiceProxy>;
...
bool CallClientStreamSayHello(const StreamForwardServiceProxyPtr& proxy, int request_count) {
...
bool CallServerStreamSayHello(const StreamForwardServiceProxyPtr& proxy) {
  auto context = ::trpc::MakeClientContext(proxy);
  ::trpc::test::helloworld::HelloRequest request;
...
bool CallBidiStreamSayHello(const StreamForwardServiceProxyPtr& proxy, int request_count) {
  auto context = ::trpc::MakeClientContext(proxy);
...
int Run() {
  ...
  auto stream_greeter_proxy = ::trpc::GetTrpcClient()->GetProxy<StreamForwardServiceProxy>(FLAGS_service_name, option);
