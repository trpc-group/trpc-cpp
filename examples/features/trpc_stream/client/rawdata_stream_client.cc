...
DEFINE_string(service_name, "trpc.test.helloworld.RawDataStreamService", "callee service name");
...
bool CallRawDataStreamReadWrite(const std::shared_ptr<::trpc::RpcServiceProxy>& proxy, int request_count) {
  ...
  context->SetFuncName("/trpc.test.helloworld.RawDataStreamService/RawDataReadWrite");
