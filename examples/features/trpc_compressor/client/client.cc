...
DEFINE_string(addr, "127.0.0.1:12345", "ip:port");
...
void Invoke(const std::shared_ptr<::trpc::test::helloworld::GreeterServiceProxy>& prx,
            ::trpc::compressor::CompressType type) {
  ::trpc::test::helloworld::HelloRequest request;
  ::trpc::test::helloworld::HelloReply reply;
  request.set_msg("compressor msg");

  auto context = ::trpc::MakeClientContext(prx);
  context->SetTimeout(1000);
  context->SetReqCompressType(type);
  // set compressor level, only valid in client
  // there are 3 compression level( skFastest kDefault kBest ),
  // the compression effect is enhanced sequentially, and the CPU usage also increases
  // if not set, use kDefault
  context->SetReqCompressLevel(::trpc::compressor::kDefault);
...
int Run() {
  ...
  Invoke(prx, ::trpc::compressor::kGzip);
  Invoke(prx, ::trpc::compressor::kSnappyBlock);
  Invoke(prx, ::trpc::compressor::kZlib);
