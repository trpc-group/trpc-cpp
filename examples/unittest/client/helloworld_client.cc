...
DEFINE_string(service_name, "trpc.test.unittest.Greeter", "callee service name");

int Run() {
  auto proxy = ::trpc::GetTrpcClient()->GetProxy<::trpc::test::unittest::GreeterServiceProxy>(FLAGS_service_name);
  test::unittest::GreeterClient greeter_client(proxy);
  std::cout << "start sayhello" << std::endl;
  return greeter_client.SayHello();
}
