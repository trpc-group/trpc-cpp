...
# https://github.com/Tencent/flare/blob/master/flare/tools/gdb-plugin.py
# This source file may have been modified by THL A29 Limited, and licensed under the BSD 3-Clause License.
"""Usage:

1. Import the plugin into GDB (execute in GDB's CLI):
   `source /path/to/gdb_fiber_plugin.py`
...
FIBER_STATE_DEAD = 4  # trpc::fiber::detail::FiberState::Dead
# @sa: fiber/detail/stack_allocator_impl.h
STACK_TOP_ALIGNMENT = 1 * 1024 * 1024
# @sa: fiber/detail/fiber_entity.h
FIBER_STACK_RESERVED_SIZE = PAGE_SIZE - 64 # pagesize - kFiberMagicSize
...
class Fiber(object):
    ...
    def _get_fiber_entity_field_offset(name):
        expr = "&(('trpc::fiber::detail::FiberEntity'*)0x0)->{0}".format(name)
        try:
...
def get_fibers(inferior):
    ...
    stacks_ptr, count = struct.unpack(
        ...
        inferior.read_memory(
            gdb.parse_and_eval("&'trpc::fiber::detail::stack_registry'"), 16))
...
class InfoFibersCommand(gdb.Command):
    ...
    def __init__(self):
        super(self.__class__, self).__init__('info fibers',
                                                gdb.COMMAND_STATUS)
...
class ListFibersCommand(gdb.Command):
    ...
    def __init__(self):
        super(self.__class__, self).__init__('list-fibers', COMMAND_USER)
...
class ListFibersCompactCommand(gdb.Command):
    ...
    def __init__(self):
        super(self.__class__, self).__init__('list-fibers-compact',
                                                       COMMAND_USER)
