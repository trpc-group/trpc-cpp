...
/// @note Parses SSE text format into SseEvent objects and serializes SseEvent objects into text.
class SseParser {
 public:
  ...
  /// @brief Serialize an SseEvent to SSE text format
  /// @param event The SseEvent to serialize
  /// @return SSE text string
  static std::string SerializeEvent(const SseEvent& event);

  /// @brief Serialize multiple SseEvent objects to SSE text format
