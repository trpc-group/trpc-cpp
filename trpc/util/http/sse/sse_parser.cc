...
std::string SseParser::SerializeEvent(const SseEvent& event) {
  std::ostringstream oss;
  
  // Add event type if specified
  if (!event.GetEventType().empty()) {
    oss << "event: " << event.GetEventType() << "\n";
  }
  
  // Add event ID if specified
  if (event.GetId().has_value()) {
    oss << "id: " << event.GetId().value() << "\n";
  }
  
  // Add retry timeout if specified
  if (event.GetRetry().has_value()) {
    oss << "retry: " << event.GetRetry().value() << "\n";
  }
  
  // Add data
  if (!event.GetData().empty()) {
    // Handle comment events (data starts with ":")
    if (event.GetData()[0] == ':') {
      oss << event.GetData() << "\n";
    } else {
      // Split data by lines and prefix each with "data: "
      auto data_lines = SplitLines(event.GetData());
...
std::string SseParser::SerializeEvents(const std::vector<SseEvent>& events) {
  std::ostringstream oss;
  
  for (const auto& event : events) {
    oss << SerializeEvent(event);
  }
...
void SseParser::ParseLine(const std::string& line, SseEvent& event) {
  ...
  if (field == "event") {
    ...
    try {
      ...
      event.SetRetry(retry_ms);
    } catch (const std::exception&) {
      // Invalid retry value, ignore
    }
  } else if (field == "data") {
    // Append to existing data with newline
    std::string current_data = event.GetData();
    if (!current_data.empty()) {
      current_data += "\n";
    }
    current_data += value;
    event.SetData(current_data);
  }
}
