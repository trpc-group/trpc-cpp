...
#define TRPC_LOG_MSG(level, msg) TRPC_STREAM(::trpc::log::kTrpcLogCacheStringDefault, level, msg)

#define TRPC_LOG_MSG_IF(level, condition, msg) \
  if (condition) {                             \
    TRPC_LOG_MSG(level, msg);                  \
  }

/// @brief uses default logger for logging with context
#define TRPC_LOG_MSG_EX(level, context, msg) \
  TRPC_STREAM_EX(::trpc::log::kTrpcLogCacheStringDefault, level, context, msg)
