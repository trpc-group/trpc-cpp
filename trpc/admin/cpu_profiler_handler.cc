...
void CpuProfilerHandler::CommandHandle(http::HttpRequestPtr req, rapidjson::Value& result,
  ...
  if (enable.empty() || (enable != "y" && enable != "n")) {
    result.AddMember("errorcode", -2, alloc);
    result.AddMember("message", "wrong parameter, please use ?enable=y/n", alloc);
    return;
  }

  if (enable == "y" && !ProfilingIsEnabledForAllThreads()) {
    ProfilerStart("cpu.prof");
  } else if (enable == "n" && ProfilingIsEnabledForAllThreads()) {
    ProfilerStop();
  }
...
const char* WebCpuProfilerHandler::SetUpdateEvents() {
  return "      var target_url = '/cmdsweb/profile/cpu_draw?seconds=' + h_seconds;\n"
         ...
         "  var req_url = '/cmdsweb/profile/cpu_draw?seconds=' + h_seconds;\n"
...
void WebCpuProfilerHandler::CommandHandle(http::HttpRequestPtr req, rapidjson::Value& result,
                                          rapidjson::Document::AllocatorType& alloc) {
  std::string html;
#ifndef TRPC_ENABLE_PROFILER
  ...
  if (GetFiles(ProfilingType::kCpu, &mfiles)) {
  ...
  if (view_name == "new_profile") {
    html.append("Profiling cpu for ");
    html.append(std::to_string(seconds));
    html.append(" seconds, please waiting ...");
  } else {
    html.append("Profiling cpu for view: ");
    html.append(view_name);
  }
  html.append("</div>\n");
  html.append("<br>\n<br>\n");
...
void WebCpuProfilerDrawHandler::CommandHandle(http::HttpRequestPtr req, rapidjson::Value& result,
                                              rapidjson::Document::AllocatorType& alloc) {
  std::string html;
#ifndef TRPC_ENABLE_PROFILER
  std::string alert_info = "alert(please recomplile with -DTRPC_ENABLE_PROFILER ";
  ...
  if (is_new_profile) {
    ...
    char prof_name[256];
    if (MakeNewProfName(ProfilingType::kCpu, prof_name, sizeof(prof_name), &mfiles)) {
      TRPC_LOG_ERROR("do MakeNewProfName fail");
      html.append("MakeNewProfName fail");
      result.AddMember(rapidjson::StringRef("trpc-html"), rapidjson::Value(html, alloc).Move(), alloc);
      return;
    }
    TRPC_LOG_INFO("start Profile, prof_name: " << prof_name);
    ProfilerStart(prof_name);
    sleep(seconds);
    ProfilerStop();
    TRPC_LOG_INFO("stop Profile,  check prof_name: " << prof_name);
    view_name = prof_name;
  } else {
  ...
  if (!use_cache_file) {
    ...
    TRPC_LOG_WARN("do cpu profile drawing used: " << end_time - begin_time);
    ...
    TRPC_LOG_WARN("cpu profile write to file used: " << end_time - begin_time);
  }
