...
#include "sliding_limiter_server_filter.h"
...
class SlidingWindowServerTestFixture : public ::testing::Test {
 public:
  static void SetUpTestCase() {
    trpc::TrpcConfig::GetInstance()->Init("./trpc/overload_control/slidingwindow_limiter/slidingwindow_overload_ctrl.yaml");
    bool ret = InitSlidingWindowServerFilter();
    ASSERT_TRUE(ret);
  }
  ...
  static bool InitSlidingWindowServerFilter() {
    MessageServerFilterPtr sliding_limiter_filter(new overload_control::SlidingWindowServerFilter());
    sliding_limiter_filter->Init();
    return FilterManager::GetInstance()->AddMessageServerFilter(sliding_limiter_filter);
  }
};

TEST_F(SlidingWindowServerTestFixture, Init) {
  MessageServerFilterPtr filter = FilterManager::GetInstance()->GetMessageServerFilter("slidingwindow_limiter");
...
TEST_F(SlidingWindowServerTestFixture, Ok) {
  MessageServerFilterPtr filter = FilterManager::GetInstance()->GetMessageServerFilter("slidingwindow_limiter");
  SlidingWindowServerFilter* time_filter = static_cast<SlidingWindowServerFilter*>(filter.get());
...
TEST_F(SlidingWindowServerTestFixture, Overload) {
  MessageServerFilterPtr filter = FilterManager::GetInstance()->GetMessageServerFilter("sliding_limiter");
  SlidingWindowServerFilter* time_filter = static_cast<SlidingWindowServerFilter*>(filter.get());
