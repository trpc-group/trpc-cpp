...
TEST(FixedWindowLimiterControlConf, All) {
  FixedWindowLimiterControlConf conf;
  ASSERT_EQ(conf.limit, 1000);
  ASSERT_EQ(conf.is_report, false);

  YAML::convert<FixedWindowLimiterControlConf> concurr_yaml;

  conf.limit = 20000;

  YAML::Node concurr_node = concurr_yaml.encode(conf);

  FixedWindowLimiterControlConf decode_conf;
