...
`runtime` 是 tRPC-Cpp 框架对线程模型的抽象，支持的 runtime 有三种：

- fiber：类似 go 语言的协程，协程可以在不同线程之间调度，即m个协程可以运行在n个线程上
...
runtime 的选择及配置参考[如何选择及配置 runtime](runtime.md#how-to-choose-and-configure-the-runtime)

常见的调用方式有两种：

- 协程模式：编程风格为同步，对应到 fiber runtime
- future/promise 模式：编程风格为异步，对应到 separate runtime 和 merge runtime

## 调用类型

根据调用协议划分，调用类型分为如下几种：

- [trpc一应一答](./trpc_protocol_client.md)
- [trpc流式](./trpc_protocol_streaming_client.md)
- [http](./http_protocol_client.md)
- [https](./https_client_guide.md)
- [grpc一应一答](./grpc_protocol_client.md)
- [grpc流式](./grpc_protocol_streaming_service.md)
- [flatbuffers](./flatbuffers_protocol_client.md)
...
框架也支持用户自行开发插件来定制服务调用的行为。拦截器是典型的使用场景，例如自定义拦截器来实现服务调用的认证和授权，调用质量的监控和上报等。另外，服务寻址是服务调用流程中非常重要的一个环节，寻址插件（selector）提供服务实例的策略路由选择，负载均衡和熔断处理能力，是客户端开发中需要特别关注的部分
...
只有纯客户端调用模式下需要用户自己获取配置文件并初始化相关参数。获取 TrpcClient 统一使用 trpc::GetTrpcClient() 接口，代码如下：
